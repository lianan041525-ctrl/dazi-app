...
const HomeIcon = ({ active }: IconProps) => (
  <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
    <path
      d="M3 10.5L12 3l9 7.5V20a1 1 0 01-1 1h-5v-7h-6v7H4a1 1 0 01-1-1v-9.5z"
    ...
    />
    <defs>
      <linearGradient id="homeGrad" x1="0" y1="0" x2="24" y2="24">
...
const MatchIcon = ({ active }: IconProps) => (
  <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
    <path
      ...
      strokeWidth="1.8"
      strokeLinejoin="round"
      fill={active ? 'url(#matchGrad)' : 'none'}
      fillOpacity={active ? 0.2 : 0}
    />
    <circle cx="18" cy="6" r="1.5" fill={active ? '#FFD700' : 'rgba(255,215,0,0.5)'} />
    <defs>
      <linearGradient id="matchGrad" x1="0" y1="0" x2="24" y2="24">
        <stop offset="0%" stopColor="#FF6B9D" />
        <stop offset="100%" stopColor="#C026D3" />
      </linearGradient>
    </defs>
...
const MineIcon = ({ active }: IconProps) => (
  <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
    ...
    />
    <defs>
      <linearGradient id="mineGrad" x1="0" y1="0" x2="24" y2="24">
        <stop offset="0%" stopColor="#FF6B9D" />
        <stop offset="100%" stopColor="#C026D3" />
      </linearGradient>
    </defs>
    ...
    >
      ...
      <span
        ...
        style={{
          ...
          textShadow: active ? '0 0 8px rgba(255,107,157,0.5)' : 'none',
        }}
      >
        {label}
      </span>
    ...
    >
      <div className="relative h-[58px] flex items-center">
        <div className="flex-1 flex h-full">
          <TabItem href="/" label="发现" icon={<HomeIcon active={!!isHome} />} active={!!isHome} />
          <TabItem href="/messages" label="消息" icon={<MessageIcon active={!!isMessages} />} active={!!isMessages} />
        </div>

        <div className="w-[68px] shrink-0" />

        <div className="flex-1 flex h-full">
          <TabItem href="/match" label="匹配" icon={<MatchIcon active={!!isMatch} />} active={!!isMatch} />
          <TabItem href="/mine" label="我的" icon={<MineIcon active={!!isMine} />} active={!!isMine} />
        </div>

        <Link
          href="/post"
          className="absolute left-1/2 -translate-x-1/2 -top-4 active:scale-95 transition-transform"
          aria-label="发布"
        >
          <div
